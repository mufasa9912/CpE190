...
int main()
{
	...
	if(file == NULL)
	{
		printf("Could not open file '_recordArrayFilterOutputInit.raw'\n");
	}
	else 
	{
	...
	if(stream)
	{
		Pa_AbortStream(stream); Pa_CloseStream(stream);
	}
...
static int paTestCallBack(const void *inputBuffer, void *outputBuffer, unsigned long framesPerBuffer, const PaStreamCallbackTimeInfo* timeInfo, PaStreamCallbackFlags statusFlags, void *userData)
{
	...
	else
	{
			...
			*writePointerRecordArray++  = *readPointerHolder;//was *readPointer++; Problem- the ++ increments
        	if(data -> outputSilenceFlag == 1)
			{
				*writePointerFilterOutArray++ = SAMPLE_SILENCE;
			}
        	else {
				*writePointerFilterOutArray++ = *readPointerHolder;
			}
